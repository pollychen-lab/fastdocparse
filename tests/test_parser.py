...
from parser import DocumentParser, _parse_json_from_llm
...
def test_tc1_1_invoice_schema_extraction():
    ...
    with patch("parser.extract_text_from_pdf", return_value="Invoice INV-123 Total 100.0. This is a very long text to avoid OCR fallback."):
...
def test_tc1_2_arbitrary_schema():
    ...
    with patch("parser.extract_text_from_pdf", return_value="It is sunny and I am happy today and everything is great"):
...
def test_tc1_3_scanned_receipt():
    ...
    with patch("parser.extract_text_from_image_ocr", return_value="Total: 15.00") as mock_ocr:
...
def test_tc1_6_missing_field():
    ...
    with patch("parser.extract_text_from_pdf", return_value="dummy dummy dummy dummy dummy dummy dummy dummy yes"):
        res = parser.extract(b"dummy", schema)
...
def test_tc2_3_cross_check_failure():
    ...
    with patch("parser.extract_text_from_pdf", return_value="total 100.0 items 20.0 50.0 and here is some extra padding text"):
...
def test_tc2_4_unparseable_date_flagged():
    ...
    with patch("parser.extract_text_from_pdf", return_value="date Not a date. Here is some more text to make it longer."):
...
def test_tc3_2_invoice_schema():
    ...
    with patch("parser.extract_text_from_pdf", return_value="Acme Corp INV-123 100.0. Added text to avoid OCR fallback."):
...
def test_tc4_1_structured_mode_enabled():
    ...
    with patch("parser.extract_text_from_pdf", return_value="dummy dummy dummy dummy dummy dummy dummy dummy") as mock_extract:
...
def test_tc4_3_ocr_structured_formatting():
    ...
    from ocr_engine import extract_text_from_image_ocr, HAS_RAPID_OCR
    ...
    with patch("ocr_engine._rapid_ocr") as mock_ocr, patch("ocr_engine.Image.open"):
...
def test_tc5_1_list_chunking():
    ...
    with patch("parser.chunk_document_text", return_value=["chunk1", "chunk2"]), \
...
def test_tc5_3_truncation_flag():
    ...
    with patch("parser.pymupdf.open", return_value=mock_doc), \
         patch("parser.extract_text_from_pdf", return_value="dummy dummy dummy dummy dummy dummy dummy dummy"), \
         patch("parser.chunk_document_text", return_value=["chunk1"]):
        res = parser.extract(b"dummy", schema)
