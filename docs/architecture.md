...
- **`__init__.py` doesn't lazy-load its re-exports beyond `ocr_engine`'s OCR engine itself** — `import docextract` costs about 0.6s, mostly from the `openai` SDK's own import graph. This was measured, not guessed (see `ocr_engine.py`'s `HAS_RAPID_OCR` — that one *is* lazy, via `importlib.util.find_spec`, because it used to add another 0.8s on its own).
